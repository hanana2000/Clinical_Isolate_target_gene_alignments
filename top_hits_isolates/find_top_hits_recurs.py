...
def get_top_hits(target_genes, isolates_path, output_folder): 
    ...
    output_folder = os.path.join(output_folder, f"results_{os.path.basename(os.path.dirname(isolates_path))}", Path(target_genes).name)
    ...
    # get a full list of all isolate proteomes
    root = Path(isolates_path)
    isolate_paths = get_iso_prots_paths(root)
    if not isolate_paths: 
        print(f"no isolate protein files found in {isolates_path}")
    # iterate through the isolates one at a time
    for iso_file in isolate_paths:
        isolate_faa, isolate_name = str(iso_file), ""
        if os.path.basename(iso_file) != "protein.faa": 
            isolate_name = os.path.basename(iso_file)
        else: isolate_name = os.path.basename(os.path.dirname(iso_file))
        database_path = f"{databases_folder}{isolate_name}_database.dmnd"
        command = ["diamond", "makedb", "--in", isolate_faa, "-d", database_path]
        print(">>", " ".join(command), "\n")
        try:
            result = subprocess.run(command, check=True)
            print(f"{isolate_name} database created successfully!\n")
        ...
        for target_folder in os.listdir(target_genes): 
            ...
            for file in os.listdir(folder_path): 
                if file.endswith(".faa"): 
                    command = ["diamond", "blastp", "-d", database_path, "-q", f"{folder_path}/{file}", "-o", f"{top_five_folder}{target_folder}_{isolate_name}.tsv", "-e", "1e-10", "--query-cover", "70", "--subject-cover", "70", "--max-target-seqs", "5", "--outfmt", "6", "qseqid", "sseqid", "pident", "length", "mismatch", "gapopen", "qstart", "qend", "sstart", "send", "evalue", "bitscore", "qlen", "slen"]
                    print("\n>>", " ".join(command), "\n")
                    try: 
...
def get_iso_prots_paths(root):
    ...
    if not flat: 
        ...
        print(f"\n[info] Found {len(flat)} .faa in {root} (recursive search).")
    return flat 
