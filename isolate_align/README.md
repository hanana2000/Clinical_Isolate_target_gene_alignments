...
This repo has two scripts for MAFFT aligning isolate protein top hits with target genes from reference genomes. 

- align_to_isolates_recurs.py: This script will align TWO SETS of target genes to the top hits generated by compare_hits_between_refs.py. It will output an alignment for each isolate per target gene, and a summary file per category of target. 

- align_to_isolates_single_recurs.py: If you have just run one set of target genes on all the isolates (therefore did not use compare_hits_between_refs.py), the align_to_isolates_single_recurs.py can be passed the isolate proteomes, the output of find_top_hits_recurs.py, and ONE SET of target genes, and output mafft alignments accordingly. 
...
## 🧪 Usage of align_to_isolates_recurs.py
...
## 🧪 Usage of align_to_isolates_single_recurs.py

If you run align_to_isolates_single_recurs.py without required arguments you will get this message: 

```bash 
usage: align_to_isolates_single_recurs.py [-h] --target_genes TARGET_GENES
                                          --path_tophits PATH_TOPHITS
                                          --isolate_proteins ISOLATE_PROTEINS
                                          --output_folder OUTPUT_FOLDER

Align isolate top hits from top_hits_isolates/find_top_hits_recurs.py to
reference target genes in ONE SET of target proteins.

options:
  -h, --help            show this help message and exit
  --target_genes TARGET_GENES
                        Path to the folder with subdirectories containing .faa
                        files with first set of reference target genes.
  --path_tophits PATH_TOPHITS
                        Path to top hits folder generated by
                        find_top_hits_recurs.py, with winners and ambiguous
                        files.
  --isolate_proteins ISOLATE_PROTEINS
                        Path to the folder with all isolate proteins GCA.
  --output_folder OUTPUT_FOLDER
...
To use this script, you must first have already run the top_hits_isolates/find_top_hits_recurs.py script of your isolate data and ONE SET of target reference genes. The output should have "winners.tsv" and "ambiguous.tsv" files. Example: 
...
```
you would pass align_to_isolates_single_recurs.py the 📁results_isolate_proteins_GCA/PAO1_target_genes folder path. The script then seraches for the top_hits_consistent folder. 

The script expects the target genes to be formated in subdirectories representing categories of genes. For example: 
...
you would pass align_to_isolates_recurs.py the PAO1_target_genes folder path.
...
You want to pass the align_to_isolates_single_recurs.py script the /ncbi_dataset/data folder. 

The align_to_isolates_single_recurs.py script can also take GCA (or GCF) .fsa_aa.gz, .fasta, or .faa files that might have been manually downloaded. it expects flat data in this stucture: 
...
You want to pass the align_to_isolates_single_recurs.py script the /isolate_proteins_GCA folder. 


## 📊 Output of align_to_isolates_recurs.py
